...
class ProductRepository extends BaseRepository
{
    ...
    public function getProductByCategoryIds($categoryIds, $params)
    {
        return $this->model->whereIn('category_id', $categoryIds)
            ->when($params && array_key_exists('brands', $params) && $params['brands'], function ($query) use ($params) {
                $brands = explode(',', $params['brands']);
                $query->whereIn('vendor', $brands);
            })->when($params && array_key_exists('price', $params) && $params['price'], function ($query) use ($params) {
                $query->where('price', '>', $params['price']);
            })->when($params && array_key_exists('size', $params)  && $params['size'], function ($query) use ($params) {
            ...
            })->when($params && array_key_exists('discount', $params) && $params['discount'], function ($query) use ($params) {
